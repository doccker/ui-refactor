 ...
 *
 * 作用域（v0.3.3）：三层防护只在 UI 会话（/redesign、/uiloop 等本插件命令）内生效；
 * off 模式（日常开发、其他 skill）完全休眠，.ai-protected-paths.txt 不影响任何普通会话。
 */
...
export default async function (pi: ExtensionAPI) {
	...
	pi.on("tool_call", async (event, ctx) => {
		// off 模式（未进入 UI 会话）完全休眠：不拦 edit/write/bash、不做契约差分。
		// .ai-protected-paths.txt 只服务本插件的 UI 工作流（/redesign、/uiloop），
		// 不得影响其他 skill / 日常开发（2026-09 实战：别的项目跑通用工作流被硬拦，
		// 用户被迫注释配置文件才能继续，v0.3.3 起改为会话内生效）。
		if (state.mode === "off") {
			return undefined;
		}
		// UI 会话内规则来源：显式配置优先，无配置用自动探测兑底
		...
		const noteGlobalStyle = (rel: string) => {
			if (!ctx.hasUI || state.globalStyleNotified.has(rel)) {
				return;
			}
		...
		if (isToolCallEventType("bash", event)) {
			...
			// guarded-bash（/ui-allow-bash 后）：UNKNOWN -> BLOCK
			const verdict = checkBashCommand(event.input.command, ctx.cwd, rules, "strict");
			if (verdict.blocked) {
				state.telemetry.track({ name: "guard_blocked", kind: "bash" });
				if (ctx.hasUI) {
		...
		if (isToolCallEventType("edit", event)) {
			...
			if (!recon.ok) {
				...
				if (recon.degraded) {
					// 降级 = 插件自身能力缺失，不代表这次编辑危险。但 UI 会话内
					// 第二层防护失能就是 UNKNOWN，按硬约束必须 BLOCK，否则「改 UI 不偷改
					// 业务逻辑」的承诺当场落空（2026-09-01 实战：全程降级，模型删掉一个
					// t() 调用，守卫本该弹确认却静默放行）。
					if (ctx.hasUI && !state.degradedNotified) {
						state.degradedNotified = true;
						ctx.ui.notify(
							`内容分析降级（${recon.reason}）。UI 会话中按 fail-closed 拦截业务文件编辑，请 /ui-end 后排查插件安装。`,
							"warning",
						);
					}
					return {
