...
import { resolvePiEntry } from "./edit-recon.ts";

/** 与 README / CHANGELOG / install-project.sh 保持一致 */
// 必须与 package.json 的 version 一致。scripts/release.sh 发版时自动改写这一行，
// smoke-test 里另有断言兜底：忘记同步会导致所有用户上报的版本号永远停在旧值，
// PostHog 里就无法按版本区分修复效果（2026-09-01 真实发生过：v0.3.1 上报成 0_3_0）。
...
export async function readPiVersion(): Promise<string> {
	try {
		// 不能用裸 import.meta.resolve：全局安装目录没有 node_modules，解析必然失败，
		// pi_version 会永远是 unknown —— degraded 告警响了也不知道是 pi 哪个版本打挂的。
		// 复用 edit-recon 里那套三级回退（同一个根因，B0-1）。
