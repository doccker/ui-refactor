	...
	pi.on("agent_settled", async (_event, ctx) => {
		...
		try {
			...
			if (useAgentCapture()) {
				...
				// 截图指令要求模型把自查截图放进 scratch/，但 playwright 不会自动建目录：
				// 2026-09-01 实战里模型第一次写 scratch 直接 ENOENT，只好先 write 一个占位文件。
				await mkdir(resolve(ctx.cwd, ".ai", "screenshots", "scratch"), { recursive: true });
